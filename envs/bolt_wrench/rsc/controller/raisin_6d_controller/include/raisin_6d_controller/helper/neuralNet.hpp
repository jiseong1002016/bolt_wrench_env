 ...
 public:
  ...
  Linear(std::vector<int> hiddenSize) {
    ...
    for (int i = 0; i < (int)params.size(); ++i) {
      if (i % 2 == 0) {
        W[i / 2].resize(architecture_[i / 2 + 1], architecture_[i / 2]);  // Eigen matrix resize not std::vector resize
...
class GRU {
 public:
  ...
  GRU(int numLayer) {
    ...
    W.resize(architecture_.size() - 1);
    b.resize(architecture_.size() - 1);
    x.resize(numLayer_ + 1);

    for (int w = 0; w < params.size(); w += 4) {
      if (w % 4 == 0) {
        if (w / 4 == 0){ /// first layer
          W[w/2].resize(3 * hiddenDim, inputDim);
          W[w/2 + 1].resize(3 * hiddenDim, hiddenDim);
          b[w/2].resize(3 * hiddenDim);
          b[w/2 + 1].resize(3 * hiddenDim);

          params[w + 0].resize(3 * inputDim * hiddenDim);
          params[w + 1].resize(3 * hiddenDim * hiddenDim);
          params[w + 2].resize(3 * hiddenDim);
          params[w + 3].resize(3 * hiddenDim);
        }
        else { /// not the first layer
          W[w/2].resize(3 * hiddenDim, hiddenDim);
          W[w/2 + 1].resize(3 * hiddenDim, hiddenDim);
          b[w/2].resize(3 * hiddenDim);
          b[w/2 + 1].resize(3 * hiddenDim);

          params[w + 0].resize(3 * hiddenDim * hiddenDim);
          params[w + 1].resize(3 * hiddenDim * hiddenDim);
          params[w + 2].resize(3 * hiddenDim);
          params[w + 3].resize(3 * hiddenDim);
        }
  ...
  void readParamFromTxt(std::string filePath) {
    ...
    for (int i = 0; i < int(params.size()); i += 4) {
      memcpy(W[i/2 + 0].data(), params[i + 0].data(), sizeof(dtype) * W[i/2 + 0].size());
      memcpy(W[i/2 + 1].data(), params[i + 1].data(), sizeof(dtype) * W[i/2 + 1].size());
      memcpy(b[i/2 + 0].data(), params[i + 2].data(), sizeof(dtype) * b[i/2 + 0].size());
      memcpy(b[i/2 + 1].data(), params[i + 3].data(), sizeof(dtype) * b[i/2 + 1].size());
  ...
  inline Output forward(Input &input) {
    ...
    /// gru
    for (int l = 0; l < numLayer_; ++l) {
      Eigen::Matrix<dtype, -1, -1>
          W_ir = W[2*l].middleRows(0*hiddenDim, hiddenDim),
          W_iz = W[2*l].middleRows(1*hiddenDim, hiddenDim),
          W_in = W[2*l].middleRows(2*hiddenDim, hiddenDim),
          b_ir = b[2*l].middleRows(0*hiddenDim, hiddenDim),
          b_iz = b[2*l].middleRows(1*hiddenDim, hiddenDim),
          b_in = b[2*l].middleRows(2*hiddenDim, hiddenDim),
          W_hr = W[2*l + 1].middleRows(0*hiddenDim, hiddenDim),
          W_hz = W[2*l + 1].middleRows(1*hiddenDim, hiddenDim),
          W_hn = W[2*l + 1].middleRows(2*hiddenDim, hiddenDim),
          b_hr = b[2*l + 1].middleRows(0*hiddenDim, hiddenDim),
          b_hz = b[2*l + 1].middleRows(1*hiddenDim, hiddenDim),
          b_hn = b[2*l + 1].middleRows(2*hiddenDim, hiddenDim);

      Eigen::Matrix<dtype, -1, 1>
          r = W_ir * x[l] + b_ir + W_hr * h[l] + b_hr,
          z = W_iz * x[l] + b_iz + W_hz * h[l] + b_hz;

      sigmoid_.nonlinearity(r);
      sigmoid_.nonlinearity(z);

      Eigen::Matrix<dtype, -1, 1>
          n = W_in * x[l] + b_in + r.cwiseProduct(W_hn * h[l] + b_hn);

      tanh_.nonlinearity(n);

      h[l] = n - z.cwiseProduct(n) + z.cwiseProduct(h[l]);
      x[l + 1] = h[l];
    }
