...
class ENVIRONMENT : public RaisimGymEnv {
 ...
 public:
  ...
  // [추가] setObStatistics를 통해 커리큘럼 팩터 받아오기
  void setObStatistics(std::map<std::string, double> stats) {
    // Python에서 "curriculum"이라는 키로 값을 보내면 여기서 받습니다.
    if(stats.find("curriculum") != stats.end()) {
        this->curriculumFactor_ = stats["curriculum"];
    }
  }
